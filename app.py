...
app = Flask(__name__)
model = pickle.load(open('elm.pickle', 'rb'))
...
def predict():
    if request.method == "POST":
        ...
        print(prediction)
        if prediction == 1:
            predictor_text = 'Patient has stroke risk'
        else:
            predictor_text = 'Congratulations, patient does not have stroke risk'
        return render_template('index.html', predictor_text=predictor_text)
